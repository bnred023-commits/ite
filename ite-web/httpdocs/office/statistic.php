...
if ($_GET[Statistic_Del]=='Delete') {
	if (isset($_GET[statistics_web_id])){
		$_SESSION[statistics_web_id] =  $_GET[statistics_web_id];
	}

	$statistics_web_id 			=   $_SESSION[statistics_web_id] ;
	$statistics_web_Del 		=	"DELETE FROM `statistics_web` WHERE statistics_web_id = '$statistics_web_id' ";
	$statistics_web_Qurey  		= 	mysqli_query($con,$statistics_web_Del);

	if($statistics_web_Qurey) {
...
$Q = 1;
$Row = "SELECT * FROM statistics_web WHERE ";
...
if (isset($_GET[keyword])&&$_GET[keyword]!='') {
	...
	$keyword = str_replace("\"","&quot;",$keyword);
	if ($Q==1) {
		$Row .= " (  statistics_web_ip LIKE '%$keyword%' OR  statistics_web_browser LIKE '%$keyword%'  OR  statistics_web_language LIKE '%$keyword%'  )";
		$Q++;
	}
	else{
		$Row .= " AND (  statistics_web_ip LIKE '%$keyword%' OR  statistics_web_browser LIKE '%$keyword%'  OR  statistics_web_language LIKE '%$keyword%'  ) ";
		$Q++;
	}
}
...
if ($Q==1) {
	...
	while ($Device 	= mysqli_fetch_array($Device_QR)) {
		...
		$StatisticLoop_SL .= "  statistics_web_browser  LIKE  '%$Device[DeviceText1]%'   " ;
...
$statistics_web_SL = $Row." ORDER BY statistics_web_id DESC LIMIT $page_Start , $Per_page ";
$statistics_web_QR 	= mysqli_query($con,$statistics_web_SL);

$YYYY = date('Y-m-d');

$statistics_web_time = time();
$statistics_web_online = time() - 300;

$sql = "select * from statistics_online where statistics_online_time > '$statistics_web_online'";
$result = mysqli_query($con,$sql);
$statistics_online = mysqli_num_rows($result);

$DaySl = $Row." AND ( statistics_web_date = '$YYYY' ) ";
		...
		<div class="row">
			...
			<div class="col-md-10">
				<div class="row">
					...
					<div class="col-md-4">
						<div class="panel panel-default">
							...
							<div class="panel-body">
								<h4>
									<? echo number_format($statistics_online); ?> คน
								</h4>
							</div>
						</div>
					</div>
						...
						<div class="panel panel-default">
							...
							<div class="panel-body">
								...
								for ($i=0; $i<=14; $i++) { 
									...
									$DaySl = $Row." AND  statistics_web_date = '$date'  ";
									$DayQuery = mysqli_query($con,$DaySl);
									$DayNum = mysqli_num_rows($DayQuery);
						...
						<div class="panel panel-default">
							...
							<div class="panel-body">
								...
								for ($i=1; $i<=12; $i++) { 
									...
									$DaySl = $Row." AND   MONTH(statistics_web_date) = '$Mounth' and YEAR(statistics_web_date) = '$Year' ";
									$DayQuery = mysqli_query($con,$DaySl);
									$DayNum = mysqli_num_rows($DayQuery);
					...
					<div class="col-md-12" id="statistics_web_last">
						<form class="form-inline" method="get" action="statistic.php#statistics_web_last">
							...
							<div class="form-group" style="margin-bottom: 15px;">
								<a href="statistic.php" class="btn btn-default">
								...
								<div class="table-responsive">
									<table class="table table-striped">
										...
										<tbody>
											<?
											$i=1;
											while ($statistics_web 	= mysqli_fetch_array($statistics_web_QR)) {
												?>
												<tr>
													<td>
													...
													<td>
														<p style="font-size: 14px;"><?php echo displaydate($statistics_web[statistics_web_date]); ?></p>
													</td>
													<td>
														<p style="font-size: 14px;"><?php echo $statistics_web[statistics_web_time]; ?></p>
													</td>
													<td>
														<p style="font-size: 14px;"><?php echo $statistics_web[statistics_web_ip]; ?></p>
													</td>
													<td title="<?php echo $statistics_web[statistics_web_browser]; ?>">
														<p style="width: 300px;font-size: 10px;" class="hide2"><?php echo $statistics_web[statistics_web_browser]; ?></p>
													</td>
													<td title="<?php echo  substr($statistics_web[statistics_web_browser],11); ?>">
														<div style="padding: 5px;">
															...
															$Device_Row 	= mysqli_num_rows($Device_QR);
															while ($Device 	= mysqli_fetch_array($Device_QR)) {
																$StatisticLoop_SL 		= " SELECT * FROM statistics_web WHERE (
																statistics_web_browser  		LIKE  '%$Device[DeviceText1]%'  OR
																statistics_web_browser  		LIKE  '%$Device[DeviceText2]%' 	OR 
																statistics_web_browser  		LIKE  '%$Device[DeviceText3]%'     )
																AND  (statistics_web_id = '$statistics_web[statistics_web_id]' )";
													...
													<td>
														<a href="statistic.php?statistics_web_id=<?php echo $statistics_web[statistics_web_id]; ?>&Statistic_Del=Delete" onclick="return confirm(' ยืนยันการลบข้อมูล ?  ')"  class="btn btn-danger btn-sm">
