...
if ($_POST['pagecontent_photo_update']) {
	if($_FILES['pagecontent_photo']['name']!=''){
		...
		if ($pagecontent_photo_Reult) {
			echo"<script>   window.location='why_us.php?UPDATE'; </script>";
		}
	}
}
...
if ($_GET[pagecontent_photo_update]=='delete') {
	...
	$pagecontent_photo_Reult = mysqli_query($con,$pagecontent_photo_Update);
	echo"<script>alert('แก้ไขเรียบร้อยแล้ว'); window.location='why_us.php?update';</script>";
}
...
if ($_POST['add_gallery']) {
	...
	if (  trim($gallery_photo)==''   &&  trim($gallery_video)==''   && trim($gallery_review)==''  &&  trim($gallery_youtube)==''  &&  trim($gallery_facebook)==''   &&  trim($gallery_link)==''   &&  trim($gallery_download)==''  ) {
		echo"  <script>alert('กรุณากรอกอย่างใดอย่างหนึ่ง '); window.location='why_us.php?#gallery';  </script>";
	}
	else{
		...
		echo"<script>  window.location='why_us.php#gallery".$_SESSION[gallery_id]."'; </script>";
	}
}
...
if ($_POST['update_gallery']) {
	...
	if ($gallery_Reult) {
		...
		echo"<script>  window.location='why_us.php?UPDATE#gallery".$gallery_id."'; </script>";
	}
}
if ($_GET['gallery_del']) {
	...
	if($gallery_Qurey) {
		echo"<script>  window.location='why_us.php?DELETE'; </script>";
	}
	else{
		echo"<script>alert('เกิดข้อผิดพลาด'); window.history.back(); </script>";
	}
										...
										<div class="modal-content">
											<form class="form" enctype="multipart/form-data" method="post">
												...
												<div class="modal-footer">
													...
													<a href="why_us.php?pagecontent_photo_update=delete" onclick="return confirm(' ยืนยันการลบข้อมูล ? ')"  class="btn btn-danger">
													...
													<td id="gallery<?php echo $gallery[gallery_id]; ?>">	
														...
														if (isset($gallery[gallery_photo])&&trim($gallery[gallery_photo])!='') {
															...
															<div class="caption" style="margin: 10px;">
																...
																<a href="why_us.php?gallery_id=<?php echo $gallery[gallery_id]; ?>&gallery_del=x" onclick="return confirm(' ยืนยันการลบข้อมูล  ? ')"  class="btn btn-sm btn-danger"><span class="glyphicon glyphicon-trash"></span> ลบ </a>
															</div>
