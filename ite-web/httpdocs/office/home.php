...
if ($_GET[pagecontent_photo_update]=='delete') {
	...
	$pagecontent_photo_Reult = mysqli_query($con,$pagecontent_photo_Update);
	echo"<script>alert('แก้ไขเรียบร้อยแล้ว'); window.location='home.php?update';</script>";
}
...
if ($_POST['add_gallery']) {
	...
	if (  trim($gallery_photo)==''   &&  trim($gallery_video)==''   && trim($gallery_review)==''  &&  trim($gallery_youtube)==''  &&  trim($gallery_facebook)==''   &&  trim($gallery_link)==''   &&  trim($gallery_download)==''  ) {
		echo"  <script>alert('กรุณากรอกอย่างใดอย่างหนึ่ง '); window.location='home.php?#gallery';  </script>";
	}
	else{
		...
		echo"<script>  window.location='home.php#gallery".$_SESSION[gallery_id]."'; </script>";
	}
}
...
if ($_POST['update_gallery']) {
	...
	if ($gallery_Reult) {
		...
		echo"<script>  window.location='home.php?UPDATE#gallery".$gallery_id."'; </script>";
	}
}
if ($_GET['gallery_del']) {
	...
	if($gallery_Qurey) {
		echo"<script>  window.location='home.php?DELETE'; </script>";
	}
	else{
		echo"<script>alert('เกิดข้อผิดพลาด'); window.history.back(); </script>";
	}
			...
			<div class="col-md-10">
				<div class="row">
					...
					<div class="col-md-6 text-right">
						<h3>
							<a href="suggestion.php" class="btn btn-primary <? if ($_SESSION['page'] =='suggestion.php') { echo 'active';} ?>">
								หัวข้อแนะนำหน้าแรก <span class="badge"><? echo $Rowsuggestion; ?></span>
							</a>
						</h3>
					</div>
					<div class="col-md-12">
				...
				<div class="row">
					...
					<div class="col-md-6">
						<div class="panel panel-default">
							<div class="panel-heading">
								รูปภาพ  : <span class="text-primary"> <? echo $pagecontent[pagecontent_topic]; ?> </span>
									...
									<div class="modal-dialog">
										<div class="modal-content">
											<form class="form" enctype="multipart/form-data" method="post">
												<div class="modal-header">
													<button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
													<h4 class="modal-title" id="exampleModalLabel">แก้ไขรูปภาพ  <span class="text-primary"> <? echo $pagecontent[pagecontent_topic]; ?> </span> </h4>
												</div>
												<div class="modal-body">
													<div class="form-group">
													...
													<td id="gallery<?php echo $gallery[gallery_id]; ?>">	
														...
														if (isset($gallery[gallery_photo])&&trim($gallery[gallery_photo])!='') {
															...
															<div class="caption" style="margin: 10px;">
																...
																<a href="home.php?gallery_id=<?php echo $gallery[gallery_id]; ?>&gallery_del=x" onclick="return confirm(' ยืนยันการลบข้อมูล  ? ')"  class="btn btn-sm btn-danger"><span class="glyphicon glyphicon-trash"></span> ลบ </a>
															</div>
