...
if ($_POST['add_gallery']) {
	...
	if (  trim($gallery_photo)==''   &&  trim($gallery_video)==''   && trim($gallery_review)==''  &&  trim($gallery_youtube)==''  &&  trim($gallery_facebook)==''   &&  trim($gallery_link)==''   &&  trim($gallery_download)==''  ) {
		echo"  <script>alert('กรุณากรอกอย่างใดอย่างหนึ่ง '); window.location='pagecontent_one.php?#gallery';  </script>";
	}
	else{
		...
		echo"<script>  window.location='pagecontent_one.php#gallery".$_SESSION[gallery_id]."'; </script>";
	}
}
...
if ($_POST['update_gallery']) {
	...
	if ($gallery_Reult) {
		...
		echo"<script>  window.location='pagecontent_one.php?UPDATE#gallery".$gallery_id."'; </script>";
	}
}
if ($_GET['gallery_del']) {
	...
	if($gallery_Qurey) {
		echo"<script>  window.location='pagecontent_one.php?DELETE'; </script>";
	}
	else{
		echo"<script>alert('เกิดข้อผิดพลาด'); window.history.back(); </script>";
	}
	...
	<div class="container-fluid">
		<div class="row">
			...
			<div class="col-md-10">
				<div class="row">
					<div class="col-md-12">
						<h3>    หน้าเพจเพิ่มเติม   : <span class="text-primary bold"> <?php echo $pagecontent[pagecontent_topic]; ?> </span>  </h3>
						<hr>
					</div>
				</div>
				...
				<div class="row">
					<div class="col-md-12 br-margin2">
						<a href="pagecontent.php?page=<? echo $page; ?>" class="btn btn-primary"><span class="glyphicon glyphicon-step-backward"></span> กลับ </a>
					...
					<div class="col-md-12">
						...
						<div class="panel panel-default">
							<div class="panel-heading">
								รายละเอียด หน้าเพจเพิ่มเติม  :  <span class="text-primary bold"> <?php echo $pagecontent[pagecontent_topic]; ?> </span>
							</div>
							<div class="panel-body">
								<div class="row br-margin2">
									<div class="col-md-12">
										<div class="form-group">
											<label class="control-label col-md-3" > ชื่อ   หน้าเพจเพิ่มเติม  </label>
						...
						<div class="panel panel-default" id="gallery">
							<div class="panel-heading">
								<div class="row">
									<div class="col-md-9">
										เนื้อหา (รูปภาพ วิดีโอ ข้อความ ไฟล์ดาวโหลด ลิ้ง)
									</div>
									<div class="col-md-3 text-right" style="margin: -5px;">
										<button  type="button" class="btn btn-sm  btn-success" data-toggle="modal" data-target="#add_gallery">
											<span class="glyphicon glyphicon-plus-sign"></span>
											เพิ่มเนื้อหา
										</button>
									</div>
								</div>
								<div id="add_gallery" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel">
									<div class="modal-dialog modal-lg" role="document">
										<div class="modal-content">
											<form action="" method="post" enctype="multipart/form-data">
												<div class="modal-header">
													<button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
													<h4 class="modal-title" id="exampleModalLabel"> เพิ่ม เนื้อหา <span class="text-primary">(ไม่จำเป็นต้องกรอกทั้งหมด)</span> </h4>
												</div>
												<div class="modal-body">
													<div class="row">
														<div class="form-group col-md-6">
															<label class="control-label" for="email">รูปภาพ </label>
															<input Type="file" class="form-control"  multiple="multiple" name="gallery_photo">
														</div>
														<div class="form-group col-md-6">
															<label class="control-label" for="email">วิดีโอ ไฟล์อัพโหลด </label>
												...
												<tr id="<?php echo $gallery['gallery_id'] ?>">
													<td id="gallery<?php echo $gallery[gallery_id]; ?>">															
														...
														<div class="caption" style="margin: 10px;">
															<button type="button" class="btn btn-info btn-sm" data-toggle="modal" data-target="#update_gallery<?php echo $gallery[gallery_id]; ?>">
																<span class="glyphicon glyphicon-edit"></span>
																แก้ไข
															</button> 
															...
															<a href="pagecontent_one.php?gallery_id=<?php echo $gallery[gallery_id]; ?>&gallery_del=x" onclick="return confirm(' ยืนยันการลบข้อมูล  ? ')"  class="btn btn-sm btn-danger"><span class="glyphicon glyphicon-trash"></span> ลบเนื้อหานี้</a>
														</div>
													</td>
												</tr>
												<?
